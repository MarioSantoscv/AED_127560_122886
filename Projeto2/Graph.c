...
Graph* GraphGetSubgraph(const Graph* g, IndicesSet* vertSet) {
  ...
  // Cria o subgrafo vazio com mesmas propriedades do original
  Graph* new = GraphCreateEmpty(g->indicesRange, g->isDigraph, g->isWeighted);

  //Adicionar ao subgrafo todos os vertices
  for (unsigned int idVertice = 0; idVertice < g->indicesRange; idVertice++) {
    if (IndicesSetContains(vertSet, idVertice)) {
      GraphAddVertex(new, idVertice);
    }
  }

  //  Para cada vértice verificar quais de suas arestas existem no subgrafo
  for (unsigned int idVertice = 0; idVertice < g->indicesRange; idVertice++) {
    if (!IndicesSetContains(vertSet, idVertice)) continue;

    // encontrar  vertice na lista de vertices do grafo original
    struct _Vertex tmp;
    tmp.id = idVertice;
    ListSearch(g->verticesList, &tmp);
    struct _Vertex* verticeAtual = ListGetCurrentItem(g->verticesList);

    List* listaArestas = verticeAtual->edgesList;
    int quantidadeArestas = ListGetSize(listaArestas);

    if (quantidadeArestas > 0) {
      ListMoveToHead(listaArestas);
      for (int idxAresta = 0; idxAresta < quantidadeArestas; ListMoveToNext(listaArestas), idxAresta++) {
        struct _Edge* aresta = ListGetCurrentItem(listaArestas);
        unsigned int idVizinho = aresta->adjVertex;

        if (!IndicesSetContains(vertSet, idVizinho)) continue;

        // Grafo nao orientado adicionar aresta apenas uma vez
        if (g->isDigraph || idVertice < idVizinho) {
          if (g->isWeighted)
            GraphAddWeightedEdge(new, idVertice, idVizinho, aresta->weight);
          else
            GraphAddEdge(new, idVertice, idVizinho);
        }
      }
    }
  }
...
IndicesSet* GraphGetSetAdjacentsTo(const Graph* g, unsigned int v) {
  ...
  // Criar um vertice temporario para fazer a search
  struct _Vertex tmp;
  tmp.id = v;
  // Procurar o vertice na lista de vertices do grafo
  ListSearch(g->verticesList, &tmp);
  struct _Vertex* vertex = ListGetCurrentItem(g->verticesList);
  List* edges = vertex->edgesList;

  // Iterar pelos edges e adicionar os vertices adjacentes ao conjunto a retornar
  int n = ListGetSize(edges);
  if (n > 0) {
    ListMoveToHead(edges);
    for (int i = 0; i < n; ListMoveToNext(edges), i++) { // com a virgula posso ter duas expressoes no mesmo for
...
double* GraphComputeVertexWeights(const Graph* g) {
  ...
  assert(g->isDigraph == 0); // Apenas para grafos não-orientados

  for (unsigned int v = 0; v < g->indicesRange; v++) {
    if (!IndicesSetContains(g->verticesSet, v))
      continue; // não é um vértice válido, mantém -1 (vertice não existe)

    // Buscar _Vertex correspondente a v
    struct _Vertex tmp;
    tmp.id = v;
    ListSearch(g->verticesList, &tmp);
    struct _Vertex* pVertex = ListGetCurrentItem(g->verticesList); //ponteiro para o vertice 

    double total = 0.0;

    List* edges = pVertex->edgesList;
    int n_edges = ListGetSize(edges);

    if (g->isWeighted) {
      // Grafo ponderado: somar os pesos
      if (n_edges > 0) {
        ListMoveToHead(edges);
        for (int i = 0; i < n_edges; ListMoveToNext(edges), i++) {
          struct _Edge* e = ListGetCurrentItem(edges);
          total += e->weight;
        }
      }
    } else {
      // Grafo não ponderado: apenas conta o número de arestas
      total = (double)n_edges;
    }
    weightsArray[v] = total;
  }

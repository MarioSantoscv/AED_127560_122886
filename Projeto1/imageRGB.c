...
Image ImageCopy(const Image img) {
  ...
  //Allocating the rows and copying the pixels
  for(uint32 i =0; i < img->height; i++){
    copy-> image[i] = AllocateRowArray(img -> width);
    memcpy(copy->image[i], img->image[i], (size_t)img->width * sizeof(uint16));
  }
...
int ImageIsEqual(const Image img1, const Image img2) {
  ...
  //compare height width and num_colors
  if(ImageWidth(img1) != ImageWidth(img2) || ImageHeight(img1) != ImageHeight(img2)){
    return 0;
  }

  uint32 h = ImageHeight(img1);
  uint32 w = ImageWidth(img1);

  for (uint32 y = 0; y < h; ++y) {
    uint16 *row1 = img1->image[y];
    uint16 *row2 = img2->image[y];
    for (uint32 x = 0; x < w; ++x) {
      uint16 l1 = row1[x];
      uint16 l2 = row2[x];

      if (l1 >= ImageColors(img1)|| l2 >= ImageColors(img2)) return 0;
  ...
  return 1;  // all pixels match in RGB 
}
...
Image ImageRotate90CW(const Image img) {
  ...
  result->num_colors = img->num_colors;
  for (uint16 i = 0; i < img->num_colors; i++) {
...
static int RegionFillRecAux(Image img, int u, int v, uint16 newLabel, uint16 targetLabel){
  ...
  if(current != targetLabel) return 0;

  //if the current is equal to the new label, nothing to do 
  ...
  //Recursively label every neighbor(up, down,right,left)
  count+=RegionFillRecAux(img, u+1, v, newLabel,  targetLabel);//right
  count+=RegionFillRecAux(img, u-1, v, newLabel,  targetLabel);//left
  count+=RegionFillRecAux(img, u, v+1, newLabel,  targetLabel);//down
  count+=RegionFillRecAux(img, u, v-1, newLabel,  targetLabel);//up
...
int ImageRegionFillingRecursive(Image img, int u, int v, uint16 label) {
  ...
  // Call recursive auxiliary
  return RegionFillRecAux(img, u, v, label, target);
...
int ImageRegionFillingWithSTACK(Image img, int u, int v, uint16 label) {
  ...
  firstpixel.u = u;
  firstpixel.v = v;
  StackPush(stack, firstpixel);

  int count = 0;
    while (!StackIsEmpty(stack)) {
    PixelCoords currentpixel = StackPop(stack);
    ...
    //push to stack the 4 neighbors(left, right, up, down)
    PixelCoords n;
    n.u = x - 1; n.v = y;     StackPush(stack, n);//left
    n.u = x + 1; n.v = y;     StackPush(stack, n);//right
    n.u = x;     n.v = y - 1; StackPush(stack, n);//up
    n.u = x;     n.v = y + 1; StackPush(stack, n);//down
  }
...
int ImageRegionFillingWithQUEUE(Image img, int u, int v, uint16 label) {
  ...
  firstpixel.u = u;
  firstpixel.v = v;
  QueueEnqueue(q, firstpixel);

  int count = 0;

  while (!QueueIsEmpty(q)) {
    PixelCoords currentpixel = QueueDequeue(q);
    int x = currentpixel.u;
    int y = currentpixel.v;
    ...
    //enqueue to queue the 4 neighbors(left, right, up, down)
    PixelCoords n;
    n.u = x - 1; n.v = y;     QueueEnqueue(q, n);
    n.u = x + 1; n.v = y;     QueueEnqueue(q, n);
    n.u = x;     n.v = y - 1; QueueEnqueue(q, n);
    n.u = x;     n.v = y + 1; QueueEnqueue(q, n);
  }
...
int ImageSegmentation(Image img, FillingFunction fillFunct) {
  ...
  int regions = 0;
  rgb_t color = 0x000000; // initial color (will be updated before use)
  ...
  for (uint32 y = 0; y < h; ++y) {
    for (uint32 x = 0; x < w; ++x) {
      ...
      if (cur == 0) { //check for WHITE pixel
        // generate next color and allocate in LUT
        color = GenerateNextColor(color);
        uint16 newLabel = (uint16)LUTAllocColor(img, color);
        // fill region using user selected function
        fillFunct(img, (int)x, (int)y, newLabel);
        regions++;
      }
    }
  }
